...
class ProductAddController extends BaseController {
	...
	public function getIndex()
	{
		...
		//	return Redirect::to('/')->with('error_message' , trans('product.not_allow_to_add_product'));

		if(CUtil::isMember() && !CUtil::isUserAllowedToAddProduct()){
		...
		$logged_user_details = CUtil::getUserDetails($logged_user_id);
		if(!$logged_user_details['shop_status'])
			return Redirect::to('myproducts')->with('error_message', trans('myProducts.your_shop_inactive_error_message'));
		...
		$p_url = URL::action('ProductAddController@postAdd');
		$action = 'add';
		$p_details = array();
		if($p_id != '' && is_numeric($p_id))
		{
			$get_common_meta_values = Cutil::getCommonMetaValues('product-edit-meta');
			...
			if(isset($p_details) && count($p_details) > 0)
			{
				if($p_details['product_user_id'] == $logged_user_id){
					try
					{
						$p_url = URL::action('ProductAddController@postEdit');
						$category_id = $p_details['product_category_id'];
						$action = 'edit';
						$d_arr['product_user_details'] = $logged_user_details;
					}
					catch (Exception $e)
					{
				...
				{
					$p_id = '';
					$error_msg = trans('product.invalid_access');
				}
			}
			else
			{
		...
		$get_common_meta_values = Cutil::getCommonMetaValues('add-product');
		if($get_common_meta_values)
		{
			$this->header->setMetaKeyword(str_replace('TAB_NAME', ucwords(str_replace('_', ' ',$tab)), $get_common_meta_values['meta_keyword']));
			$this->header->setMetaDescription(str_replace('TAB_NAME', ucwords(str_replace('_', ' ',$tab)), $get_common_meta_values['meta_description']));
			$this->header->setMetaTitle(str_replace('TAB_NAME', ucwords(str_replace('_', ' ',$tab)), $get_common_meta_values['meta_title']));
		}
		if($action == 'edit' && isset($p_details['product_name']))
		{
			$get_common_meta_values = Cutil::getCommonMetaValues('edit-product');
			if($get_common_meta_values)
			{
				$this->header->setMetaKeyword(str_replace(array('TAB_NAME', 'PRODUCT_NAME'), array(ucwords(str_replace('_', ' ',$tab)), $p_details['product_name']), $get_common_meta_values['meta_keyword']));
				$this->header->setMetaDescription(str_replace(array('TAB_NAME', 'PRODUCT_NAME'), array(ucwords(str_replace('_', ' ',$tab)), $p_details['product_name']), $get_common_meta_values['meta_description']));
				$this->header->setMetaTitle(str_replace(array('TAB_NAME', 'PRODUCT_NAME'), array(ucwords(str_replace('_', ' ',$tab)), $p_details['product_name']), $get_common_meta_values['meta_title']));
			}
		}
		...
		elseif($tab == 'shipping')
		{
			...
			$ship_templates_list = $this->shipping_template_service->getShippingTemplatesList();
			...
			{
				$ship_template_id = $this->shipping_template_service->getDefaultShippingTemplate();
				if($ship_template_id == '') {
		...
		elseif($tab == 'tax')
		{
			$taxations = array('' => trans('product.select_a_tax'));
			...
			$taxationslist = Webshoptaxation::Taxations()->getTaxations(array('user_id' => $user_id), 'list', array('order_by' => 'tax_name', 'sort_type' => 'asc'));
		...
		elseif($tab == 'stocks')
		{
			$is_downloadable_product = isset($p_details['is_downloadable_product'])? $p_details['is_downloadable_product']: 'No';
			if($is_downloadable_product == 'Yes')
			{
				//To redirect to next tab..
				$new_tab_key =  $this->productAddService->getNewTabKey($tab, $p_id);
				if($new_tab_key == '')
					return Redirect::to('product/add?id='.$p_id.'&p=basic');
				else
					return Redirect::to('product/add?id='.$p_id.'&p='.$new_tab_key);
			}
		...
		elseif($tab == 'cancellation_policy')
		{
			...
			$d_arr['cancellation_policy'] = $cancellation_policy->getCancellationPolicyDetails($logged_user_id);
		...
		elseif($tab == 'status')
		{
			...
			$d_arr['user_account_balance'] = CUtil::getUserAccountBalance($logged_user_id);
		}
		$d_arr['p'] = $tab;
		...
		{
			return Redirect::to('product/add')->with('error_message', trans('common.invalid_access'));
		}
		...
		return View::make('addProduct', compact('cancellation_policy_value', 'd_arr', 'section_arr', 'p_details', 'stock_details', 'p_id', 'p_url', 'category_main_arr', 'category_sub_arr', 'service_obj', 'action', 'category_id', 'admin_user_code', 'variations_obj'));
	}
	...
	public function postAdd()
	{
		if(Input::has('add_product'))
		{
			...
			$user_section_id = (isset($input_arr['user_section_id']) && $input_arr['user_section_id'] != '') ? $input_arr['user_section_id']: 0;
			...
			else
			{
				...
				return Redirect::to('product/add?id='.$add_product_id.'&p=price');
			}
			if($error_msg != '')
			{
				return Redirect::to('product/add')->with('error_message', $error_msg)->withInput();
			}
		}
		...
		switch($action)
		{
			...
			case 'upload_resource_preview': // images on the image tab
				...
				if($resource_count < Config::get('webshoppack.preview_max'))
				{
					...
					if ($upload_status['status'] == 'success')
					{
						$resource_arr = array(	'product_id'=>$p_id,
			...
			case 'upload_swap_image': // images on the image tab
				if(isset($this->variations_service))
				{
					...
					if($resource_count < Config::get('variations::variations.swap_img_max'))
					{
						...
						if ($upload_status['status'] == 'success')
						{
							$swap_image_arr = array('item_id'=>$p_id,
													'filename'=>$file_info['filename_no_ext'],
													'ext'=>$file_info['ext'],
													'title'=>$file_info['title'],
													'width'=>$file_info['width'],
			...
			case 'edit_shipping':
				...
				try
				{
					...
					{
						echo json_encode(array(	'result'=>'success','shipping_id'=> $shipping_id, 'error_msg' => trans('product.fee_not_changed')));
					}
				}
				catch(Exception $e)
				{
				...
				try
				{
					if($taxation_id > 0)
					{
						...
						if($taxatonid)
						{
							...
							echo json_encode(array(	'result'=>'success','taxation_id'=> $taxation_id, 'error_msg' => trans('product.fee_not_changed')));
						}
					}
					else{
						echo json_encode(array(	'result'=>'failed','taxation_id'=> $taxation_id, 'error_msg' => trans('product.select_valid_taxation')));
					}
				}
				catch(Exception $e)
				{
				...
				try
				{
					if($taxation_id > 0)
					{
						...
						if(isset($taxation_det) && $taxation_det!='' && count($taxation_det) > 0)
						{
							...
							echo json_encode(array(	'result'=>'failed','taxation_id'=> $taxation_id, 'error_msg' => trans('product.fee_details_not_avail')));
						}
					}
					else{
						echo json_encode(array(	'result'=>'failed','taxation_id'=> $taxation_id, 'error_msg' => trans('product.select_valid_taxation')));
			...
			case 'load_group_variations_list':
				if(isset($this->variations_service))
				{
					...
					$result_arr = $this->variations_service->populateVariationsInGroupByGroupId($group_id, $product_id);
					$var_resource_options_arr = $result_arr['var_resource_options_arr'];
					$var_show_cancel_button = $result_arr['var_show_cancel_button'];
					echo View::make('variations::manageItemsVariationsAttributesBlock', compact('var_resource_options_arr', 'var_show_cancel_button'));
				}
				exit;
				break;
			...
			case 'getMatrixDetails':
				if(isset($this->variations_service))
				{
					...
					//echo URL::asset(Config::get('variations::variations.swap_img_folder'));exit;
					/*echo '<pre>';print_r($item_matrix_details_arr);
					exit;*/

					echo View::make('variations::matrixItemUpdateBlock', compact('item_matrix_details_arr'));
					/*
					if($item->getFormField('mul_mat_edit'))
					{
						$variations_det_arr = $item->variation_obj->populateDefaultMatrixDetails($item->getFormField('matrix_ids'), $item->getFormField('item_id'));
						$variations_det_arr['multiple_edit'] = 1;
						if($item->getFormField('edit_field') != '')
							$variations_det_arr['edit_field'] = $item->getFormField('edit_field');
						else
							$variations_det_arr['edit_field'] = 'all';
					}
					else
					{
						$variations_det_arr = $item->variation_obj->populateMatrixDetails($item->getFormField('matrix_id'), $item->getFormField('item_id'));
						$variations_det_arr['multiple_edit'] = 0;
						$variations_det_arr['edit_field'] = 'all';
					}

					$smartyObj->clear_assign("item_matrix_details_arr");
					$smartyObj->assign('item_matrix_details_arr', $variations_det_arr);
					$smartyObj->assign('tabindex', 2000);
					setTemplateFolder('members/', 'marketplace');
			        $smartyObj->display('matrixItemUpdateBlock.tpl');
					*/
			...
			case 'set_default_matrix':
				if(isset($this->variations_service))
				{
					...
					echo View::make('variations::manageItemsAttribMatrix', compact('d_arr', 'p_id'));
	        	}
				exit;
				break;
			...
			case 'update_matrix_details':
				...
				if(isset($this->variations_service))
				{
		    		...
		    		if($edit_all == 1 OR $edit_field == 'edit_swap_image' OR isset($inputs_arr['matrix_swap_img_id']))
			    		$data_arr['swap_img_id'] = $inputs_arr['matrix_swap_img_id'];
					...
					if($valid_matrix_update == 1)
		    		{
						...
						echo View::make('variations::manageItemsAttribMatrix', compact('d_arr', 'p_id'));
//						setTemplateFolder('members/', 'marketplace');
//			        	$smartyObj->display('manageItemsAttribMatrix.tpl');
			        	$resp_html = ob_get_contents();
						ob_end_clean();
			        	echo json_encode(array('status'=>'success', 'op_html'=>$resp_html));
			        }
	...
	public function getProductSubCategories()
	{
		if(Request::ajax())
		{
			...
			if(is_numeric($category_id) && $category_id > 0)
			{
				...
				{
					$disp_result .= '<span id="span_'.$category_id.'" class="fn_subCat_'.$category_id.'"></span>';
					$disp_result .= '<div class="row fn_subCat_'.$category_id.'">';
					$disp_result .= '<div class="col-md-8 fn_subCat_'.$category_id.'"><select id="sub_category_'.$category_id.'" name="sub_category_'.$category_id.'" class="form-control bs-select fn_subCat_'.$category_id.'" onchange="listSubCategories(\'sub_category_'.$category_id.'\', \''.$category_id.'\');">';
		...
		if(Input::has('edit_product'))
		{
			...
			if($tab == 'basic')
			{
				...
				else
				{
					...
					if($new_tab_key == '')
					{
						return Redirect::to('product/add?id='.$input_arr['id'].'&p=status');
					}
					else
					{
						//To redirect to next tab..
						return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$new_tab_key);
					}
				}
				if($error_msg != '')
				{
					//return Redirect::to('product/add')->with('error_message', trans('common.correct_errors'))->withInput();
			...
			elseif($tab == 'price')
			{
				...
				if($error_msg != '')
				{
					//return Redirect::to('product/add')->with('error_message', trans('common.correct_errors'))->withInput()->withErrors($validator);
					return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', $error_msg)->withInput();
				}
			}
			elseif($tab == 'stocks')
			...
			elseif($tab == 'tax')
			{
				...
				try
				{
					...
					{
						$inputs = array('taxation_id' 	=> $inputs['taxation_id'],
						...
						if($taxatonid)
						{
							return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('success_message', trans('product.tax_added_success'));
						}
						else
						{
							return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', trans('common.some_problem_try_later'))->withInput();
						}
					}
					else
					{
			...
			elseif($tab == 'attribute')
			{
				...
				if($validator->passes())
				{
					...
					if($return_data != '')
					{
						return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', $return_data)->withInput();
					}
					else
					{
						$new_tab_key =  $this->productAddService->getNewTabKey($input_arr['p'], $input_arr['id']);
				...
				{
					return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', trans('common.correct_errors'))->withInput()->withErrors($validator);
				}
			}
			elseif($tab == 'preview_files')
			{
			...
			{
				$redirectKey = 'status';
				if(CUtil::chkIsAllowedModule('variations') && isset($this->variations_service))
				{
					$insert_arr = array();
					...
					{
						$defaultChk 	= ($this->variations_service->chkIsDefaultMatrixExist($input_arr['id'])) ? 1: 0;
						$stockChk 		= ($this->variations_service->chkIsVariationStockExist($input_arr['id'])) ? 1: 0;

						if($defaultChk && $stockChk)
						{
							$new_tab_key = $this->productAddService->getNewTabKey($input_arr['p'], $input_arr['id']);
						...
						else
						{
							if(!$defaultChk){
								...
								return Redirect::to('product/add?id='.$input_arr['id'].'&p=variations')->with('error_message', $errMsg);
							}
							if(!$stockChk){
								$errMsg = Lang::get('variations::variations.variation_stock_none_err');
								return Redirect::to('product/add?id='.$input_arr['id'].'&p=variations')->with('error_message', $errMsg);
							}
						}
					}
				...
				//To redirect to next tab..
				return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$redirectKey);
				// return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', $error_msg)->withInput();
			}
			elseif($tab == 'download_files')
			{
			...
			elseif($tab == 'cancellation_policy')
			{
				...
				else
				{
					...
					else
					{
						...
						if ($v->fails())
						{
							return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->withErrors($v)->withInput();
						}
						else
						{
			...
			elseif($tab == 'status')
			{
				...
				if(isset($input_arr['product_notes']) && $input_arr['product_notes'] != '')
				{
				...
				if($input_arr['edit_product'] == 'publish' || $input_arr['edit_product'] == 'send_for_approval')
				{
					$product = Products::initialize($input_arr['id']);
					...
					if(!Config::get('products.paid_listing')) {
						$p_details = $product->getProductDetails(0, false);
						if($p_details['date_expires'] == '0000-00-00 00:00:00' || ( $p_details['date_expires']!='0000-00-00 00:00:00' && $p_details['date_expires'] != '9999-12-31 00:00:00' && (strtotime($p_details['date_expires']) < strtotime(date('Y-m-d'))) ))
						{
					...
					$save_details = $product->save();
					$json_data = json_decode($save_details, true);
					if($json_data['status'] == 'error')
					{
						foreach($json_data['error_messages'] AS $err_msg)
						{
					...
					else
					{
						...
						{
							$prod_details = $product->getProductDetails();
							if($prod_details['product_status'] == 'Ok' || $prod_details['product_status'] == 'ToActivate')
							{
								$this->productService->sendProductMailToUserAndAdmin($input_arr['id']);
							}
							//If all the completed, then redirect to product list page
							$msg = $json_data['success_msg'];
							$final_success = true;
							return Redirect::to('myproducts')->with('success_message', $msg)->with('final_success', $final_success);
						}
					}
				}
				else if($input_arr['edit_product'] == 'update')
				{
					...
					{
						$details = $product->validateUpdate();
						$json_data = json_decode($details, true);
						if($json_data['status'] == 'error')
						{
							if(is_array($json_data['error_messages']))
				...
				else if($input_arr['edit_product'] == 'set_to_draft')
				{
					...
					{
						$product->changeStatus('Draft');
						$msg = trans('product.products_set_to_draft_success_msg');
						$final_success = true;
						return Redirect::to('myproducts')->with('success_message', $msg)->with('final_success', $final_success);
					}
				}
				else if($input_arr['edit_product'] == 'pay_from_credit')
				{
					$product = Products::initialize($input_arr['id']);
					$p_details = $product->getProductDetails();
					...
					if(Config::get('products.paid_listing')) {
						if($p_details['date_expires'] == '0000-00-00 00:00:00' || ( $p_details['date_expires']!='0000-00-00 00:00:00' && $p_details['date_expires'] != '9999-12-31 00:00:00' && (strtotime($p_details['date_expires']) < strtotime(date('Y-m-d'))) )) {
							$total_amount_to_pay = Config::get('products.product_listing_fee');
						}
					}
					if(CUtil::chkIsAllowedModule('featuredproducts')) {
						$featured_product_service = new FeaturedProductsService();
						if(isset($input_arr['plan']) && $input_arr['plan'] > 0) {
							$plan_details = $featured_product_service->getFeaturedProductsPlanSettings($input_arr['plan']);
							if(count($plan_details) > 0) {
								$plan_details = (array) $plan_details;
								$total_amount_to_pay = $total_amount_to_pay + $plan_details['featured_price'];
							}
							else {
								$error_msg = trans('featuredproducts::featuredproducts.featured_prodcut_plan_not_exit');
								return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', $error_msg)->withInput();
							}
						}
					}
					if($user_account_balance['amount'] < $total_amount_to_pay) {
						$error_msg = trans('product.insufficient_wallet_balance');
						return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$input_arr['p'])->with('error_message', $error_msg)->withInput();
					}
					...
					if(Config::get('products.paid_listing')) {
						...
						else
						{
							...
							$credit_obj->setAmount(Config::get('products.product_listing_fee'));
							...
							$transaction_arr['user_id'] = $logged_user_id;
							$transaction_arr['transaction_type'] = 'debit';
							$transaction_arr['amount'] = Config::get('products.product_listing_fee');
							$transaction_arr['currency'] = Config::get('generalConfig.site_default_currency');
							$transaction_arr['transaction_key'] = 'product_listing_fee';
							$transaction_arr['reference_content_table'] = 'product';
							$transaction_arr['reference_content_id'] = $input_arr['id'];
							$transaction_arr['status'] = 'completed';
							$transaction_arr['transaction_notes'] = 'Debited product listing fee from wallet for listed product id: '.$input_arr['id'];
							...
							$transaction_arr['transaction_notes'] = 'Credited product listing fee to wallet for listed product id: '.$input_arr['id'];
							$trans_id = $trans_obj->addNewTransaction($transaction_arr);
						}
					}
					...
					if(CUtil::chkIsAllowedModule('featuredproducts')) {
						if(isset($input_arr['plan']) && $input_arr['plan'] > 0) {
							...
							if(count($plan_details) > 0) {
								$plan_details = (array) $plan_details;
								$featured_product_service->updateFeaturedProdcutExpiryDate($p_details, $plan_details);
								$featured_product_service->setFeaturedProdcutTransaction($p_details, $plan_details);
							}
						}
					}
					return Redirect::back()->with('success_message', trans('product.fee_paid_success'));
				}
				...
				if(isset($json_data['status']) && $json_data['status'] == 'error')
				{
					...
					if($new_tab_key == '')
					{
						return Redirect::to('product/list')->with('success_message', 'Product updated successfully!!!');
					}
					else
					{
						//To redirect to next tab..
			...
			/*if(isset($input_arr['p']) && $input_arr['p'] == 'download_files')
			{
				if($this->productService->validateDownloadTab($input_arr['id']))
				{
					...
					if($new_tab_key == '')
					{
						...
						return Redirect::to('product/add?id='.$input_arr['id'].'&p=status')->with('success_message', $msg);
					}
					else
					{
						//To redirect to next tab..
				...
				{
					return Redirect::to('product/add?id='.$input_arr['id'].'&p=download_files')->with('error_message', trans('common.correct_errors'));
				}
			}
			else
			{
				...
				if($validator->passes())
				{
					...
					if($update_product_arr['status'])
					{
						...
						if($new_tab_key == '')
						{
							//If all the completed, then redirect to product list page
							$msg = (empty($this->productAddService->alert_message))? '' : trans('product.'.$this->productAddService->alert_message);
							return Redirect::to('product/add?id='.$input_arr['id'].'&p=status')->with('success_message', $msg)->with('validate_tab_arr', $validate_tab_arr);
						}
						else
						{
							//To redirect to next tab..
							return Redirect::to('product/add?id='.$input_arr['id'].'&p='.$new_tab_key)->with('validate_tab_arr', $validate_tab_arr);
						}
					}
					else
					{
						return Redirect::to('product/add?id='.$input_arr['id'].'&p=status')->with('validate_tab_arr', $validate_tab_arr);
					}
				}
				else
				{
	...
	public function postShippingCostEstimate()
	{
		...
		return View::make('referenceShippingCost', compact('d_arr'));
	}
	...
	//Variation functions start
	public function getProductSwapImagesList()
	{
		if(isset($this->variations_service)) {
			$inputs = Input::all();
			$product_id = $inputs['product_id'];
			$r_fnname = $inputs['r_fnname'];
			if(isset($this->variations_service)) {
				$swap_images_list = $this->variations_service->getProductSwapImagesList($product_id);
				$data_arr['swap_img_folder'] = URL::asset(Config::get('variations::variations.swap_img_folder'));
				$data_arr['r_fnname'] = $r_fnname;
				//echo '<pre>';print_r($swap_images_list);
				return View::make('searchSwapImage', compact('swap_images_list', 'data_arr'));
			}
		}
	}
}