...
@if(count($companies) > 0)
    ...
    <tr class="clsShippingCompany {{ $tabname }}" id="{{ $company_block }}">
        <td>
            <label class="checkbox-inline pos-relative">
        ...
        <td class="col-md-6">
        	<div>
                <ul class="list-unstyled shipping-list clearfix">
                    ...
                    @if($company->is_standard_fee_available)
                        ...
                        <li id="{{ $discount_group }}">
                            {{ Form::label($discount_name, Lang::get('shippingTemplates.discount'), array('class' => 'control-label pull-left margin-right-10')) }}
                ...
                <ul class="list-unstyled shipping-list clearfix margin-0">
                    <li><label for="{{ $delivery_type_name.'_promised' }}" class="control-label">{{Lang::get('shippingTemplates.delivery_time')}}: </label></li>
                    <li>
                        <div class="radio-list">
                            <label class="control-label">
                                <input type="radio" name="{{ $delivery_type_name }}" id="{{ $delivery_type_name.'_promised' }}" value="2"  @if($is_checked_delivery_type == 2) checked="checked" @endif/> <label for="{{ $delivery_type_name.'_promised' }}">{{ Lang::get('shippingTemplates.promised_time') }} </label>
                            </label>
                        </div>
        ...
        <td>
            <p class="margin-0 pos-relative">
				<label class="radio-inline">
					<input type="radio" name="{{ $fee_type_name }}" {{ $is_checked_fee_type }} id="{{ $fee_type_name.'_free' }}" value="3" @if($is_checked_fee_type == 3) checked="checked" @endif /> <label for="{{ $fee_type_name.'_free' }}">{{Lang::get('common.free')}} </label>
				</label>
			</p>
                ...
                <p  class="margin-0 pos-relative">
					<label class="radio-inline">
						...
						<label for="{{ $delivery_type_name.'_custom' }}">
							@if(isset($id))
								<a onclick="openCustomDeliveryTimePopup('{{URL::action('ShippingTemplateController@getCustomDeliveryTime')}}', {{$company->id}})" href="javascript:;" data-id="{{$company->id}}">{{Lang::get('shippingTemplates.custom_delivery_time')}}</a>
							@else
								<a onclick="openCustomDeliveryTimePopup('{{URL::action('ShippingTemplateController@getCustomDeliveryTime')}}', {{$company->id}})" href="javascript:;" data-id="{{$company->id}}">{{Lang::get('shippingTemplates.custom_delivery_time')}}</a>
							@endif
						</label>
        ...
        <td width="100">
            @if($company->is_custom_fee_available)
                <label class="radio-inline pos-relative">
                    ...
                    @if(isset($id))
                        <a onclick="openCustomShippingTemplatePopup('{{URL::action('ShippingTemplateController@getCustomShippingTemplate')}}', {{$company->id}})" href="javascript:;" data-id="{{$company->id}}">{{Lang::get('shippingTemplates.custom')}}</a>
                    @else
                        <a onclick="openCustomShippingTemplatePopup('{{URL::action('ShippingTemplateController@getCustomShippingTemplate')}}', {{$company->id}})"  rel="screenshots_group" href="javascript:;" class="fn_fancyboxview" data-id="{{$company->id}}">{{Lang::get('shippingTemplates.custom')}}</a>
                    @endif
                 </label>
			@endif
        </td>
