	...
	<div style="line-height:18px;">
		@if(isset($shop_order_details) && count($shop_order_details) > 0)
			...
			<p style="background:#f2dede; border:1px solid #ebccd1; border-radius:4px; color:#a94442; font:bold 14px Arial, Helvetica, sans-serif; margin:10px 0 20px; padding:10px 15px;">
				{{trans('mail.product_delivered_details')}}
			</p>
			<!-- END: ALERT BLOCK -->
			...
			<!-- BEGIN: SELLER INVOICE DETAILS -->
				<div style="line-height:18px; padding:10px 20px; background:#fafafa;">
					...
					<p style="margin:10px 0 20px 0; padding:0 0 5px 0; font:bold 16px Arial, Helvetica, sans-serif; color:#333; border-bottom:1px solid #eee;">{{trans('mail.delivered_details')}} :</p>
					<table width="98%" cellspacing="0" cellpadding="0" border="0">
						@foreach($shop_order_details as $shop_order)
							...
							@if(isset($product_details['is_downloadable_product']) && $product_details['is_downloadable_product']=='No')
								...
								<tr>
									...
									<td valign="top" align="left"><p style="padding:0;margin:0 0 8px 0;color:#1a1a1a; font:normal 13px Arial, Helvetica, sans-serif;">{{ $company_name }}</p></td>
								</tr>
								...
								<tr>
									...
									<td valign="top" align="left">
									<p style="padding:0;margin:0 0 8px 0;color:#1a1a1a; font:normal 13px Arial, Helvetica, sans-serif;">{{ $shop_order->item_qty }}</p></td>
								</tr>
								<tr>
				                    <td width="200" valign="top" align="left">
				                        <p style="padding:0; margin:0 0 8px 0; color:#707070; font:normal 13px Arial, Helvetica, sans-serif;">{{trans('mail.delivered_date')}}:</p>
				                  </td>
				                    <td valign="top" align="left">
										<p style="padding:0;margin:0 0 8px 0;color:#1a1a1a; font:normal 13px Arial, Helvetica, sans-serif;">{{ CUtil::FMTDate($delivered_date, "Y-m-d H:i:s", "") }}</p>
									</td>
				                </tr>
				                <tr>
				                	<td colspan="2">&nbsp;</td>
					...
					@if($feedback == 'yes')
						...
						<a target="blank" href="{{Url::action('FeedbackController@getAddFeedback', array($shop_order->id))}}">{{trans('mail.give_your_feedback')}}</a></p>
					@endif
				</div>
