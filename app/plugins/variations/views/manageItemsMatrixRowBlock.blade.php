...
<td>
    ...
    @else
        <span id="matrix_no_swap_img_{{$matrix_row_details['matrix_id']}}">{{ Lang::get('variations::variations.no_swap_image') }}</span>
    @endif
    ...
    @if($matrix_row_details['is_active'] == 1)
        ...
        @endif
        <a href="{{ $matrix_row_details['changeMatrixSwapImgLink'] }}" title="{{ Lang::get('variations::variations.change') }}" class="fn_updSwapImg">{{ Lang::get('variations::variations.change') }}</a>
    @endif
</td>
<td class="action-btn">
	<p>
