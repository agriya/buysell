            ...
            <table class="table table-bordered">
                 <thead>
                    <th>
                        <div class="checkbox">
                            <label>
                                {{ Form::checkbox('variationlist_sel_ckbox', '', '', array('id'=>'variationlist_sel_ckbox', 'class'=>'') ) }} 
                            ...
                            <tr>
                                <td>
                                    <div class="checkbox checkbox-inline">
                                        <label for="variation_{{ $val['variation_id'] }}">
                                ...
                                <td>
                                    @if(count($val['options_arr']) > 0)
                                        @foreach($val['options_arr'] as $varOpt)
                                            <div class="checkbox checkbox-inline">
                                                <label for="attribs_{{$varOpt['attribute_id']}}">
                                        ...
                                        @endforeach
                                    @else
                                        {{ Lang::get('variations::variations.no_options') }}
                                    @endif
                                </td>
                            </tr>
                        @endforeach
...
<div class="form-group">
	<div class="col-md-offset-2 col-md-9 margin-bottom-20">
        @if(isset($var_resource_options_arr) && count($var_resource_options_arr) > 0)	
            <button name="edit_product" id="edit_product" value="Next" type="submit" class="btn btn-sm btn-success">{{ Lang::get('variations::variations.set_variation_values') }}</button>
        @endif
